...
function defaultProfile(email) {
  const isFounderEmail = FOUNDER_EMAILS.includes(email.toLowerCase().trim());
  return {
    email,
    display_name: isFounderEmail ? 'Henrry Moyses García Rojas' : email.split('@')[0],
...
export const localAuthAdapter = {
  async me() {
    ...
    const isFounderEmail = FOUNDER_EMAILS.includes(email.toLowerCase().trim());
    if (isFounderEmail && (profile.display_name !== 'Henrry Moyses García Rojas' || !profile.is_founder)) {
      profile.display_name = 'Henrry Moyses García Rojas';
