...
import { Mic, MicOff, Keyboard, Settings2, LogOut, Power, Hand, PhoneCall, Activity } from 'lucide-react';
...
import DiagnosticPanel from '@/components/vivi/DiagnosticPanel';
...
export default function Vivi() {
  const { avatarState, avatarGesture, avatarEmotion, caption, listening, audioLevel, toggleMic, cancelSpeech, stopListening, updateSettings, user, vivi, deliverGreeting, hasPendingGreeting, online } = useVivi();
  ...
  return (
    ...
    <div className="fixed inset-0 overflow-hidden overscroll-none bg-[#0A0A0C] text-white">
      ...
      <AnimatePresence>
        {!online && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -50 }}
            className="absolute top-0 left-0 right-0 z-50 bg-rose-600/90 backdrop-blur-md text-white text-center py-2 px-4 text-xs font-semibold flex items-center justify-center gap-2 shadow-lg"
            style={{ paddingTop: 'calc(0.5rem + env(safe-area-inset-top))' }}
          >
            <span className="w-2 h-2 rounded-full bg-white animate-ping"></span>
            <span>Sin conexión: El micrófono y las respuestas de Vivi se reanudarán al conectarte.</span>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {/* Top-left: Diagnostic Panel button */}
      <div className="absolute left-4 z-30 flex items-center gap-2" style={{ top: 'calc(0.75rem + env(safe-area-inset-top))' }}>
        <button
          onClick={() => setDiagnosticOpen(true)}
          className="p-3 rounded-full bg-indigo-500/10 backdrop-blur-md border border-indigo-500/20 hover:bg-indigo-500/20 transition-all touch-manipulation flex items-center gap-1.5"
          aria-label="Diagnóstico"
        >
          <Activity className="w-5 h-5 text-indigo-400 animate-pulse" />
          <span className="text-[10px] uppercase font-bold text-indigo-300 pr-1 hidden sm:inline-block">Diagnóstico</span>
        </button>
      </div>
