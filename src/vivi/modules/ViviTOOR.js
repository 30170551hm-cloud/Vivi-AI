...
// Phase 2 Modular Tools
import WorkspaceIntegrationsTool from '../tools/WorkspaceIntegrationsTool';
import GoogleMapsPlatformTool from '../tools/GoogleMapsPlatformTool';
import UniversalCalculatorTool from '../tools/UniversalCalculatorTool';
import SecuritySandboxTool from '../tools/SecuritySandboxTool';
import McpProtocolTool from '../tools/McpProtocolTool';
import VectorStorageRagTool from '../tools/VectorStorageRagTool';
...
export default class ViviTOOR extends ModuleBase {
  ...
  async init(registry) {
    ...
    // Register Phase 2 dynamic engines
    this.registerTool(new WorkspaceIntegrationsTool());
    this.registerTool(new GoogleMapsPlatformTool());
    this.registerTool(new UniversalCalculatorTool());
    this.registerTool(new SecuritySandboxTool());
    this.registerTool(new McpProtocolTool());
    this.registerTool(new VectorStorageRagTool());
  ...
  listTools(context) {
    ...
    const disabledTools = settings?.getPrefs()?.disabled_tools || [];

    return Array.from(this._tools.values()).map((t) => {
      const isEnabled = t.enabled && !disabledTools.includes(t.name);
      return {
        name: t.name,
        ...
        retries_limit: t.retries,
        metrics: typeof t.getMetrics === 'function' ? t.getMetrics() : null,
      };
    });
  }
...
- params_json_string: un string JSON válido que contenga los parámetros requeridos para la herramienta, o un string vacío si no se requieren parámetros.
...
Si la solicitud es una conversación normal, saludo, o pregunta que Vivi puede responder con su conocimiento, usa "none".`,
        response_json_schema: {
          ...
          properties: {
            tool: { type: 'string' },
            action: { type: 'string' },
            params_json_string: { type: 'string' },
            reason: { type: 'string' },
          },
      ...
      let params = {};
      if (response && response.params_json_string) {
        try {
          params = JSON.parse(response.params_json_string);
        } catch (e) {
          this._diag('Failed to parse params_json_string, fallback to empty object');
        }
      }
      ...
      return { tool: 'none', reason: 'detection failed', params: {} };
    }
  }
  ...
  async execute(toolName, params, context) {
    ...
    try {
      // Phase 2 resilient execution via tool.run()
      const result = typeof tool.run === 'function'
  ...
  _logAction(toolName, params, result, status, durationMs, error) {
    const entry = {
      ...
      input_summary: safeJsonStringify(params || {}).slice(0, 500),
      output_summary: result?.data ? safeJsonStringify(result.data).slice(0, 500) : '',
      status,
