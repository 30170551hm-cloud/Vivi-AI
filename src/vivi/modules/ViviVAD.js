...
// RMS threshold for detecting speech — optimized to prevent ambient noise or speaker leakage
// from triggering premature barge-in, while remaining sensitive to real user voice.
...
// Period during which barge-in is ignored right after starting detection (in ms).
// This allows browser acoustic echo cancellation (AEC) filters to warm up and adapt,
// and prevents speech onset clicks/reverberations from triggering self-barge-in.
const BARGE_IN_IMMUNITY_MS = 1500;
...
export default class ViviVAD extends ModuleBase {
   ...
   */
  async startBargeInDetection(isWebSpeech = false) {
    if (this._active) return;
    if (typeof window === 'undefined' || !navigator.mediaDevices?.getUserMedia) return;

    this._isWebSpeech = isWebSpeech;
  ...
  /**
   * Update the WebSpeech mode dynamically on fallback.
   */
  setWebSpeech(isWebSpeech) {
    this._isWebSpeech = isWebSpeech;
    this._diag('Barge-in web speech mode updated', { isWebSpeech });
  }
  ...
  _poll() {
    ...
    const now = Date.now();
    const elapsedSinceStart = now - this._detectionStartTime;
    const isImmune = elapsedSinceStart < BARGE_IN_IMMUNITY_MS;

    const activeThreshold = this._isWebSpeech ? 0.18 : BARGE_IN_THRESHOLD;

    if (rms > activeThreshold) {
      if (isImmune) {
        // Under the immunity window (e.g. first 1.5s), don't allow barge-in trigger.
        // This gives local speaker sounds or start-of-speech echoes time to be canceled by the AEC filter.
        this._aboveThresholdSince = 0;
      } else {
        if (this._aboveThresholdSince === 0) {
          ...
          this._diag('Barge-in detected', { rms: rms.toFixed(4), threshold: activeThreshold.toFixed(4), elapsedSinceStart });
